...
/** Static DRAM-layout guard for the VTA baremetal path.
  ...
  *
  * Known limitation: [[audit]] passes `None` for the reference dir, sizing the
  * raw-input scratch region as 0 rather than from the real input_nn.bin as
  * [[fpga.host.GenNnBaremetal.generate]] does. An input_nn.bin over 4096 bytes
  * therefore shifts every CPU-scratch address in the generator's map but not in
  * this one, and the two silently diverge.
  *
