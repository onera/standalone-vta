...
- Create a Vivado project at `modules/fpga/synthesis/build/project/<BOARD>`.
- Run synthesis, implementation, and export the hardware handoff file (`.xsa`) to `modules/fpga/synthesis/build/vta_<BOARD>.xsa`.
...
1. **Build the Application:**
   Open the Vitis Unified IDE, import/open the workspace located at `modules/fpga/software/build/vitis_proj`, and compile the application component.
...
3. **Run the Inference via Host Script (from `modules/fpga/software`):**
...
This script generates an IP repository directory at `ip_repo/modules/` containing the IP definition.
...
make gen-test_gemm
```
This generates `modules/fpga/software/gen/init_dram.h`.
...
4. **Create the Application Component:**
   ...
   - **Source Files:** Select **Add folders** and add the following directories one by one:
     - `modules/fpga/software/driver/src/`
     - `modules/fpga/software/driver/include/`
     - `modules/fpga/software/apps/test_gemm/`
     - `modules/fpga/software/gen/` (provides `init_dram.h`)
