...
def check_bin(dtype=np.int8, debug=False, rtl=False):
    # Three dirs: the compiler dir holds the metadata, the reference dir holds
    # the golden, the simulator dir holds the result (final_output*.bin).
    ...
    if ref_raw.size != total_elements:
        raise Exception(
            f"ERROR: Reference size {ref_raw.size} does not match shape {shape} (Total: {total_elements})"
        )
        return
    ...
    if debug == True or isCorrect == False:
        ...
        if not isCorrect:
            ...
            print(
                f"Statistics: max={max_diff}, min={min_diff}, abs_error={max_abs_diff}"
            )
            print(f"-" * 30)
    ...
    if not isCorrect:
        if max_abs_diff < 5 and percentage < 5.0:
            raise Exception(
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2 or len(sys.argv) > 3:
        raise Exception(
            f"\nERROR: Require 2-3 inputs and there are {len(sys.argv)}! \n"
        )
    ...
    # Define debug
    debug = sys.argv[1] == "true" or sys.argv[1] == "True"
    if len(sys.argv) >= 3:
        rtl = sys.argv[2] == "true" or sys.argv[2] == "True"
    else:
        rtl = False

    # Run the generic inference code
    check_bin(dtype=dtype, debug=debug, rtl=rtl)
