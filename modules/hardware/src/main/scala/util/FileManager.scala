...
object FileManager {
  ...
  def getFileOrResource(path: String, fromResources: Boolean = false) = if (
    fromResources
  ) {
  ...
  def getFileOrResourceAsStream(path: String, fromResources: Boolean = false) =
    Try {
      if (fromResources) {
  ...
  def getConfigFile(path: String, fromResources: Boolean = false) = if (
    !fromResources
  ) {
  ...
  def getCompilerOutputFile(path: String, fromResources: Boolean = false) = if (
    !fromResources
  ) {
    val hwRoot = sys.env.getOrElse("MILL_WORKSPACE_ROOT", "")
    val projectRoot = new File(hwRoot)
