...
# -------------
def reference_computation(vta_config_dict, operations_dict, debug=False):
    ...
    if "GEMM" in operations_dict:
        if type(gemm_op[2]) == int:
            ...
            matrices_dict[output_name] = matrices_dict[gemm_op[0]] + np.matmul(
                matrices_dict[gemm_op[1]].astype(acc_dtype),
                matrices_dict[gemm_op[2]].astype(acc_dtype),
            )
    ...
    for alu in alu_list:
        ...
        if alu_op == "ADD_ACC":
            ...
            for i in range(0, nb_loop):
                ...
                matrices_dict[output_name] = perform_alu_operations(
                    matrices_dict[output_name],
...
# ----------------------
def perform_alu_operations(
    matrix, alu_operation="MAX_IMM", dst_idx=0, elem2=0, isIMM=True
):
    """
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        raise Exception(
            "ERROR: The arguments must be <config_file> <json_file> ... \n\n"
        )
    ...
    for vta_ir in sys.argv[3:]:
        ...
        reference_computation(vta_config_dict, operations_dict, debug=debug)
