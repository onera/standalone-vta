...
object GenNnBaremetal {
  ...
  def generate(
    ...
    val dep = CompilerOutputParser.loadDependencyInfo(depPath)
    println(
    ...
    val activeL: Seq[Model.LayerInfo] =
      if (emitCheck) {
        ...
        val alignedBase = vta.parsers.CompilerOutputParser.alignPage(allocTop2)
        println(
      ...
      case Some(o) =>
        ...
        Model.runOrExit {
          generate(
            ...
            ddrBase = vta.util.Hex.parseAddr(o.ddrBase),
            cfg = cfg,
            ...
            sdDir = o.sdDir,
            maxAddr = o.maxAddr.map(vta.util.Hex.parseAddr),
            verbose = o.verbose
          )
        }
