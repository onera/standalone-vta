...
object AuditDram {
  ...
  def audit(
    ...
    val dep =
      CompilerOutputParser.loadDependencyInfo(s"$compDir/dependency.csv")
  ...
  def main(args: Array[String]): Unit =
    OParser.parse(argParser, args, Opts()) match {
      case Some(o) =>
        ...
        val ddrBase = vta.util.Hex.parseAddr(o.ddrBase)
        val maxAddr = o.maxAddr.map(vta.util.Hex.parseAddr)
