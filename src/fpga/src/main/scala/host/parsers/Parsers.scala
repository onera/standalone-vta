...
import vta.parsers.VtaConfig
import vta.models.CompilerOutputModel.{DependencyInfo, Region}
...
object ConfigParser {
  def load(configJsonPath: String): ConfigParams = {
    val cfg = VtaConfig
      .loadRawEntriesAt(configJsonPath)
      .toOption
...
/** Builds the per-layer [[Model.LayerInfo]] list from a compiler-output dir.
  *
  * Reuses [[vta.parsers.CompilerOutputParser]] for the memory_addresses regions
  * and their gap-based byte sizes. Missing buffer types (e.g. a MaxPool layer
  * has no INP/WGT) become zero-size regions so downstream emitters can treat
  * every layer uniformly. `vtaSuffixes` come from `dep.executionOrder` and each
  * layer's `reshapeInfo` from `dep.layers` (defaulting to "im2row" when
  * absent).
  */
object LayerParser {
  def collectLayers(
    ...
    val parsed = CompilerOutputParser.loadLayerRegions(
      compDir,
      vtaSuffixes,
    ...
    parsed.map { case (suffix, regions) =>
      ...
      val mem = DataType.names.map { t =>
        t -> byName.getOrElse(t, Region(t, 0L, 0L, 0L))
      }.toMap
