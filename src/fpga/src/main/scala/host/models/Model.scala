...
import vta.models.CompilerOutputModel.Region
...
/** Host-generator data model and path/name helpers.
  *
  * Compiler-output parsing and byte-sizing are reused from
  * [[vta.parsers.CompilerOutputParser]] (memory_addresses regions and
  * dependency.csv); this object only adds the baremetal-specific layer wrapper,
  * static-load policy, and C/asm formatting helpers. Translated from
  * nnbaremetal/model.py.
  */
object Model {
  ...
  /** A VTA layer: its compiler suffix, per-buffer regions (keyed by buffer
    * type; missing buffers are `Region(name, 0, 0, 0)`), backing binary paths,
    * and the reshape policy. `reshapeInfo` defaults to "im2row" so an
    * unannotated layer keeps the conv behaviour (ACC = static bias).
    */
  case class LayerInfo(
      suffix: String,
      mem: Map[String, Region],
      binFiles: Map[String, String],
      reshapeInfo: String = "im2row",
      // Isolation-check golden region, populated only with --emit-layer-check
      // (written by DebugEmit.assignLayerCheckRegions). None in non-check mode.
  ...
  ): Seq[(Int, LayerInfo, String, Region)] =
    for {
