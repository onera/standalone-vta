...
/** Lowers an [[ExecPlan]] (raw inputs) into the resolved [[PlanStep]] IR plus
  ...
  * resolution, reshape/int32 pre-step derivation, and the per-processor payload.
  * Rendering then becomes a pure `PlanStep => String` pass.
  */
        ...
        val main: Seq[PlanStep] = if (processor == "vta") {
          ...
          val rescale: Seq[PlanStep] =
            if (logOutWidth > 3 && vtaIdx >= 0 && ldOpt.isDefined) {
              ...
              Seq(RescaleStep(stepIdx, layerName, outAddr, nElems, ld.scale, ld.offsetC))
            } else Seq.empty
          vta +: rescale
