...
-----
    python3 tools/gen_nn_baremetal.py <compiler_output_dir>             \
        ...
        [--out-data-header src/fpga/software/examples/nn_ddr_data.h]    \
...
-------
  ...
  nn_ddr_data.h   — Optional: ELF-embedded static model data (no INP/OUT)
"""
...
def gen_exec_plan_header(
    ...
    for _, processor, layer_name in dep_info.execution_order:
        ...
        out_w = (ld.tensor_w + ld.pad[1] + ld.pad[3] - ld.kw) // ld.sh + 1
...
# ---------------------------------------------------------------------------
# ELF data header (static model data + input_nn.bin, no INP/OUT per layer)
# ---------------------------------------------------------------------------


def gen_data_header(
    layers: List[LayerInfo], ddr_base: int, comp_dir: str, out_path: str
) -> None:
    """
    Embed INSN/UOP/WGT/ACC for all VTA layers plus input_nn.bin as uint32_t
    arrays.  INP (per-layer formatted input) and OUT are excluded: only
    input_nn.bin (the raw network input) is embedded for the INP region.
    """
    L: List[str] = []
    L.append("/* Auto-generated by tools/gen_nn_baremetal.py — DO NOT EDIT */")
    L.append("#pragma once")
    ...
    L.append("")
    L.append("/* --- static model data: INSN / UOP / WGT / ACC --- */")

    region_entries: List[Tuple[int, str, int]] = []

    def _embed(raw: bytes, phys: int, var_name: str) -> None:
        remainder = len(raw) % 4
        if remainder:
            raw = raw + b"\x00" * (4 - remainder)
        wc = len(raw) // 4
        L.append(f"/* @ {hex32(phys)} ({wc} words) */")
        L.append(f"static const std::uint32_t {var_name}[{wc}] = {{")
        words_per_row = 8
        for w in range(0, wc, words_per_row):
            vals = " ".join(
                f"0x{int.from_bytes(raw[j * 4 : (j + 1) * 4], 'little'):08X}u,"
                for j in range(w, min(w + words_per_row, wc))
            )
            L.append(f"    {vals}")
        L.append("};")
        L.append("")
        region_entries.append((phys, var_name, wc))

    for i, layer in enumerate(layers):
        cname = safe_c_name(layer.suffix, i)
        for buf_type in STATIC_LOAD_ORDER:
            ...
            if raw:
                _embed(
                    raw,
                    ddr_base + layer.mem[buf_type].offset,
                    f"nn_{cname}_{buf_type.lower()}",
                )

    # input_nn.bin → scratch area (ARM applies im2row at runtime into VTA INP)
    input_nn_path = os.path.join(comp_dir, "input_nn.bin")
    if os.path.isfile(input_nn_path):
        with open(input_nn_path, "rb") as bf:
            raw = bf.read()
        raw_phys = scratch_addr(layers, ddr_base)
        L.append("/* --- raw network input (scratch for ARM format-input step) --- */")
        _embed(raw, raw_phys, "nn_input_nn_raw")
    else:
        print(f"WARNING: {input_nn_path} not found — input_nn.bin not embedded")

    # Region table
    L.append("/* --- DDR region table --- */")
    L.append("struct NnDdrRegion {")
    L.append("    std::uint32_t        dst_addr;")
    L.append("    const std::uint32_t *data;")
    L.append("    std::uint32_t        word_count;")
    L.append("};")
    L.append("")
    L.append("static const NnDdrRegion nn_ddr_regions[] = {")
    for phys, var_name, wc in region_entries:
        L.append(f"    {{ {hex32(phys)}, {var_name}, {wc}u }},")
    L.append("};")
    L.append("")
    L.append("#define NN_DDR_REGION_COUNT \\")
    L.append("    (sizeof(nn_ddr_regions) / sizeof(nn_ddr_regions[0]))")
    L.append("")

    with open(out_path, "w") as f:
        f.write("\n".join(L) + "\n")
    total = sum(wc for _, _, wc in region_entries)
    print(
        f"[gen] ELF data header written to {out_path} "
        f"({len(region_entries)} regions, {total * 4} bytes)"
    )
...
# ---------------------------------------------------------------------------
# SD card file manifest header
# ---------------------------------------------------------------------------


def gen_sd_map_header(
    layers: List[LayerInfo], ddr_base: int, comp_dir: str, out_path: str
) -> None:
    """
    Generate nn_sd_map.h: an NnSdFile array mapping SD card filenames to DDR
    addresses.  INSN/UOP/WGT/ACC files are listed per layer (ACC skipped if
    absent); input_nn.bin is appended last at the scratch address.
    """
    raw_phys = scratch_addr(layers, ddr_base)

    entries: List[Tuple[str, int, int]] = []  # (sd_path, dst_addr, byte_size)
    ...
    for layer in layers:
        for buf_type in STATIC_LOAD_ORDER:
            ...
            addr = ddr_base + layer.mem[buf_type].offset
            entries.append((f"0:/{fname}", addr, size))

    input_nn_path = os.path.join(comp_dir, "input_nn.bin")
    if os.path.isfile(input_nn_path):
        entries.append(("0:/input_nn.bin", raw_phys, os.path.getsize(input_nn_path)))
    else:
        print(f"WARNING: {input_nn_path} not found — input_nn.bin entry skipped")
    ...
    L.append("")
    L.append(f"#define NN_SD_SCRATCH_ADDR  {hex32(raw_phys)}")
    L.append("")
    L.append("typedef struct {")
    L.append("    const char    *path;       /* FatFs path on SD card  */")
    L.append("    uint32_t       dst_addr;   /* DDR physical address   */")
    L.append("    uint32_t       byte_size;  /* file size in bytes     */")
    L.append("} NnSdFile;")
    L.append("")
    L.append(f"#define NN_SD_NUM_FILES  {len(entries)}u")
    L.append(f"static const NnSdFile nn_sd_files[NN_SD_NUM_FILES] = {{")

    for j, (sd_path, addr, size) in enumerate(entries):
        comma = "," if j < len(entries) - 1 else ""
        L.append(f'    {{ "{sd_path}", {hex32(addr)}, {size}u }}{comma}')
    ...
    with open(out_path, "w") as f:
        f.write("\n".join(L) + "\n")
    print(f"[gen] SD map header written to {out_path} ({len(entries)} files)")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="PATH",
        help="Output path for ELF-embedded data header (nn_ddr_data.h)",
    )
    parser.add_argument(
        "--out-sd-map",
        default=None,
        metavar="PATH",
        help="Output path for the SD card file manifest header (nn_sd_map.h)",
    )
    parser.add_argument(
        "--ps-init",
        default=None,
    ...
    for opt in (args.out_exec_plan, args.out_data_header, args.out_sd_map):
        if opt:
    ...
    if args.out_data_header:
        gen_data_header(layers, ddr_base, comp_dir, args.out_data_header)
    if args.out_sd_map:
        gen_sd_map_header(layers, ddr_base, comp_dir, args.out_sd_map)
