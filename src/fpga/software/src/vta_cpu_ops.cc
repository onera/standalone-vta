...
void run_qadd(const NnQaddStep &d)
{
    ...
    if (d.sC == 0.0f) { xil_printf("[vta] run_qadd: zero output scale\r\n"); return; }
...
/* Channel-concat for VTA block layout: for each row-block, copy each input's
   channel-blocks sequentially so output has the correct [rb, cb_A, cb_B, …] order.
   A simple flat append would be wrong for H*W > 16 (multiple row-blocks). */
void run_concat(const NnConcatStep &d)
{
    if (d.out_scale == 0.0f) { xil_printf("[vta] run_concat: zero output scale\r\n"); return; }
    ...
    for (std::uint32_t rb = 0u; rb < n_rb; ++rb) {
        for (int p = 0; p < d.nb_inp; ++p) {
            ...
            const auto *src = reinterpret_cast<const std::int8_t *>(
                static_cast<std::uintptr_t>(d.inp[p])) + rb * n_cb * 256u;
            std::int8_t *dst = out
                + (rb * tot_cb + static_cast<std::uint32_t>(p) * n_cb) * 256u;
            for (std::uint32_t cb = 0u; cb < n_cb; ++cb) {
                ...
                for (std::uint32_t e = 0u; e < 256u; ++e) {
                    std::int32_t q = static_cast<std::int32_t>(
                        std::nearbyintf(rescale_p * static_cast<float>(
                            static_cast<std::int32_t>(s_blk[e]) - d.zps[p])))
                        + d.out_zp;
                    d_blk[e] = clamp_i8(q);
                }
            }
...
{
    if (d.scale == 0.0f) { xil_printf("[vta] run_quant: zero scale\r\n"); return; }
        ...
        for (std::uint32_t obc = 0u; obc < K_blocks; ++obc) {
            ...
            for (std::uint32_t t = 0u; t < 16u; ++t) {
                ...
                for (std::uint32_t r = 0u; r < 16u; ++r) {
                    ...
                    const std::int32_t w_in = static_cast<std::int32_t>(w_out * sw + kj) - pl;
