...
int run_layer(std::uintptr_t vcr_base, const LayerDesc &layer, int timeout) {
    // 1. Flush input regions so the VTA (AXI master) sees up-to-date DDR contents.
    //    Instruction buffer: flush + invalidate (same pattern as test_gemm).
    Xil_DCacheFlushRange(static_cast<UINTPTR>(layer.insn_addr), layer.insn_count * 16u);
    Xil_DCacheInvalidateRange(static_cast<UINTPTR>(layer.insn_addr), layer.insn_count * 16u);
    ...
    //    ptr[0] = insn_addr (absolute), ptr[1..5] = ddr_base (offsets in instructions).
    VTARegs config{};
    ...
    // 4. Poll finish flag (REG_CTRL bit 1 = 0x2).
    int count = 0;
    ...
    Xil_DCacheInvalidateRange(static_cast<UINTPTR>(layer.out_phys), layer.out_bytes);
    ...
    if (layer.reloc_bytes > 0u) {
        ...
        Xil_DCacheFlushRange(static_cast<UINTPTR>(layer.reloc_dst), layer.reloc_bytes);
    }
