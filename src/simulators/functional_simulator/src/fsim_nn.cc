...
int run_nn(const FsimOptions &opts) {
  ...
  for (int i = 0; i < nb_steps; ++i) {
    // Get the name
    std::string layer_to_execute =
        get_csv_value(dependency_map, std::to_string(i), 2);
    ...
    if (layers_map.find(layer_to_execute) == layers_map.end()) {
      ...
      if (debug)
        printf("   -> Auto-created context for CPU layer: %s\n",
  ...
  for (const std::string &layer_name : execution_order) {
    ...
    std::string processor =
        get_csv_value(dependency_map, ctx.suffix.c_str(), 1);
    // Reshape
    std::string reshape_info =
        get_csv_value(dependency_map, ctx.suffix.c_str(), 2);

    if (debug)
      printf("\t On %s with reshape %s \n", processor.c_str(),
             reshape_info.c_str());

    // Offsets (A, B, C)
    int offsetA =
        strToInt(get_csv_value(dependency_map, ctx.suffix.c_str(), 3));
    int offsetB =
        strToInt(get_csv_value(dependency_map, ctx.suffix.c_str(), 5));
    int offsetU =
        strToInt(get_csv_value(dependency_map, ctx.suffix.c_str(), 7));
    int offsetV =
        strToInt(get_csv_value(dependency_map, ctx.suffix.c_str(), 9));
    int offsetC =
        strToInt(get_csv_value(dependency_map, ctx.suffix.c_str(), 25));

    // Scale (A, B, C)
    float scaleA =
        strToFloat(get_csv_value(dependency_map, ctx.suffix.c_str(), 4));
    float scaleB =
        strToFloat(get_csv_value(dependency_map, ctx.suffix.c_str(), 6));
    float scaleU =
        strToFloat(get_csv_value(dependency_map, ctx.suffix.c_str(), 8));
    float scaleV =
        strToFloat(get_csv_value(dependency_map, ctx.suffix.c_str(), 10));
    float scaleC =
        strToFloat(get_csv_value(dependency_map, ctx.suffix.c_str(), 26));
    // Rescaling factor (Sa*Sb/Sc)
    float scale =
        strToFloat(get_csv_value(dependency_map, ctx.suffix.c_str(), 27));

    // INPUT tensor shape
    int tensor_channel =
        strToInt(get_csv_value(dependency_map, ctx.suffix.c_str(), 11));
    int tensor_height =
        strToInt(get_csv_value(dependency_map, ctx.suffix.c_str(), 12));
    int tensor_width =
        strToInt(get_csv_value(dependency_map, ctx.suffix.c_str(), 13));

    // OUTPUT tensor shape
    int out_tensor_channel =
        strToInt(get_csv_value(dependency_map, ctx.suffix.c_str(), 22));
    int out_tensor_height =
        strToInt(get_csv_value(dependency_map, ctx.suffix.c_str(), 23));
    int out_tensor_width =
        strToInt(get_csv_value(dependency_map, ctx.suffix.c_str(), 24));
    ...
    // Nb of inputs
    int nb_inp =
        strToInt(get_csv_value(dependency_map, ctx.suffix.c_str(), 29));

    if (debug)
      printf("\t %d inputs: ", nb_inp);

    if (nb_inp < 1 || nb_inp > 4) {
      std::cerr << "ERROR: Too many inputs for layer " << ctx.suffix
                << std::endl;
      return EXIT_FAILURE;
    }
    ...
    if (reshape_info == "int32") {
      ...
      if (nb_inp == 1) {
        ...
        // Rescale
        std::vector<acc_dtype> reshaped_dep =
            convert_vector_type<acc_dtype>(dep_out);

        // Offset
        if (offsetA != 0) {
          reshaped_dep = subtract_offset(reshaped_dep, offsetA);
        }
        // Check if any padding is required (p0=Top, p1=Left, p2=Bottom,
        // p3=Right)
        if (p0 > 0 || p1 > 0 || p2 > 0 || p3 > 0) {
          if (debug)
            printf("\t -> Applying int32 padding: T=%d, L=%d, B=%d, R=%d\n", p0,
                   p1, p2, p3);

          reshaped_dep = pad_matrix(reshaped_dep,     // input_data
      ...
      // There are two inputs
        ...
        // Convert in int 32
        std::vector<acc_dtype> reshaped_dep1 =
            convert_vector_type<acc_dtype>(dep_out);
        std::vector<acc_dtype> reshaped_dep2 =
            convert_vector_type<acc_dtype>(dep2_out);
        ...
        if (p0 > 0 || p1 > 0 || p2 > 0 || p3 > 0) {
          ...
          reshaped_dep1 = pad_matrix(reshaped_dep1,    // input_data
                                     tensor_channel,   // tensor_channel
                                     tensor_height,    // tensor_height
                                     tensor_width,     // tensor_width
                                     block_size,       // block_size
          ...
          reshaped_dep2 = pad_matrix(reshaped_dep2,    // input_data
                                     tensor_channel,   // tensor_channel
                                     tensor_height,    // tensor_height
                                     tensor_width,     // tensor_width
                                     block_size,       // block_size
      ...
      // Rescale
      std::vector<inp_dtype> rescaled_dep =
          convert_vector_type<inp_dtype>(dep_out);

      // Reshape
      ctx.inpA = reshape(rescaled_dep,     // prev_vector
                         block_size,       // block_size
                         1,                // batch_size
                         tensor_channel,   // tensor_channel
                         tensor_height,    // tensor_height
    ...
    // DEQUANTISE
      ...
      if (debug)
        printf("\t -> Processing DequantizeLinear (CPU)\n");
      ...
      // Dequantise
      std::vector<float> float_res =
          dequantize_linear(rescaled_dep, scaleA, offsetA);
    ...
    // QUANTISE
      ...
      if (debug)
        printf("\t -> Processing QuantizeLinear (CPU)\n");
    ...
    // CONVTRANSPOSE
      ...
      if (debug)
        printf("\t -> Processing ConvTranspose (CPU)\n");

      // Get the binaries
      std::string fileCTWgtPath =
          construct_path("weight" + ctx.suffix + ".bin");
      std::string fileCTAccPath =
      ...
      // ConvTranspose
      // Inputs: Vector -> Reconstruct Tensor (using input dims) -> Conv ->
      // Flatten
      std::vector<float> float_res =
          conv_transpose(float_dep_out, CTwgt, CTacc, 1, tensor_channel,
                         tensor_height, tensor_width, // Input Dims
                         out_tensor_channel, out_tensor_height,
                         out_tensor_width, // Output Dims
                         kh, kw, sh, {p0, p1, p2, p3}, block_size);
    ...
    if ((processor != "dequant") && (processor != "convtranspose")) {
      ...
      ctx.res = rescaling(ctx.outC, // vector (int32)
  ...
  // Write result
  output_tensor(ctx.res,            // output vector
                block_size,         // block_size
                1,                  // batch_size
                tensor_channel,     // tensor_channel
                tensor_height,      // tensor_height
