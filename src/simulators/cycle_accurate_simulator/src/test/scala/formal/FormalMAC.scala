...
class MacFormalSpec_Computation_flopOut extends Module {
  ...
  assert(
    io.a === rA,
    "Signal rA should be equal to port A (when flopIn is false)"
  )
  assert(
    io.b === rB,
    "Signal rB should be equal to port B (when flopIn is false)"
  )
  assert(
    io.c === rC,
    "Signal rC should be equal to port C (when flopIn is false)"
  )
  ...
  assert(
    addV === past(add),
    "addV should be the previous state of add (when flopIn is false)"
  )
  assert(
    addV === past((mult) +& rC),
    "addV should be the previous mult +& rC (when flopIn is false)"
  )

  // Functional verification on the input-output
  assert(
    io.y === past((io.a * io.b) +& io.c),
    "Y should be the computation of the previous input signals"
  )
}
...
class MacFormalSpec_Overflow(makeDut: => MAC) extends Module {
  ...
  val YminValue = (-(1 << (outBits - 1))).S(
    outBits.W
  ) // (2^n) => 1 << n OR: 1L << n (1L = Long integer) to avoid overflow
  ...
  assert(
    (io.a * io.b) >= multMinValue && (io.a * io.b) <= multMaxValue,
    "There is an overflow on mult"
  )
  ...
  assert(
    (mult +& io.c) >= addMinValue && (mult +& io.c) <= addMaxValue,
    "There is an overflow on add"
  )
}

/** Execute Formal test
  */
class MacFormalTester
    extends AnyFlatSpec
