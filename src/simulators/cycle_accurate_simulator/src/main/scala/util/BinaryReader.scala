...
object BinaryReader {
  ...
  /** Definition of an Enumeration listing the data types. The attributes of
    * each type are its ID, the size of its vectors (in Bytes) and its
    * bit-length
    */
  ...
  def printBytes(filePath: String): Unit = {
    readBinaryFile(filePath, fromResources = true) match {
      ...
      case Failure(exception) =>
        println(
          s"Error while printing non-reversed file ${filePath} : ${exception.getMessage}"
        )
    }
  }

  /** Group the instructions in 16-Byte groups, reverse Little Endian for each
    * instruction
    * @param binaryData
    *   an array containing the bytes extracted from the binary file
    * @param dataType
    *   the type of data in the binary file
    * @return
  ...
  /** Reads the base memory addresses of the data and UOP inside a .csv file and
    ...
    * @param filePath
    *   the path to the CSV file or file name if not in a resource folder
    * @param fromResources
    *   boolean that is true if the files are in a Resources folder, false
    *   otherwise
    * @return
  ...
  /** Compute the logical addresses associated with each instruction in a Map
    * @param filePath
    *   the path to the resource file or file name if not in a resource folder
    * @param dataType
    *   the type of data in the binary file
    * @param baseAddress
    *   base address of a data type
    * @param fromResources
    *   boolean that is true if the files are in a Resources folder, false
    *   otherwise
    * @return
    *   a Map(Address, Array) that associates the logical address of a vector
    *   with its values
    */
    ...
    val groupedBinaryData =
      readBinaryFile(newFilePath, fromResources) match {
        case Success(fileContent) =>
          ...
          ) // Bytes are extracted (and reversed depending on data type) from binary file
        case Failure(exception) =>
          println(
            s"Error while grouping data (if reversal) : ${exception.getMessage}"
          )
          Failure(exception)
      }
    val baseAddrBigInt =
      ...
      case Success(data) =>
        val flattenedBits = // Flattened array containing all the bits of the binary file after LE reversal
          for {
            ...
            String
              .format("%8s", java.lang.Integer.toBinaryString(byte & 0xff))
              .replace(' ', '0')
          }
        ...
        val groupedBits = flattenedBits
          .flatMap(_.toList.map(_.toString))
          .grouped(sizeOfElement)
          .toArray
        // Returns an array with nbValues groups of size precision
        val reversePrecision =
          Map(0 -> 10, 1 -> 11, 2 -> 11) // For UOP reversal
        def groupByElemSize(arr: Array[String], index: Int): Array[String] = {
          if (index < dataType.nbValues) {
            if (dataType.id == UOP.id)
              Array(
                arr.take(reversePrecision(index)).mkString
              ) ++ groupByElemSize(arr.drop(reversePrecision(index)), index + 1)
            else
              Array(
                arr.take(dataType.precision(index)).mkString
              ) ++ groupByElemSize(
                arr.drop(dataType.precision(index)),
                index + 1
              )
          } else
            arr
        ...
        val correctedVectorsBits =
          vectorsBits.map(elem =>
            if (dataType.id == UOP.id) elem.reverse else elem
          )
        // Converts the bits to signed Int8
        def convertToInt8Signed(hexArray: Array[String]): Array[BigInt] = {
          hexArray.filter(_.nonEmpty).map { hex =>
            val bitLength = hex.length
            require(
              bitLength == 8 || bitLength == 16 || bitLength == 32,
              s"Binary length should be 8, 16 or 32-bit but is $bitLength"
            )
        ...
        val result = {
          if (map.size % 2 != 0 && dataType.id == UOP.id) { // If the number of UOPs is odd, add an empty one to the map
            map + (baseAddrBigInt + BigInt(4 * map.size) -> Array(0, 0, 0).map(
              BigInt(_)
            ))
          } else {
            map
  ...
  /** Print the addresses and values of a Map with the data encoded in a format
    * CHISEL can read
    * @param map
