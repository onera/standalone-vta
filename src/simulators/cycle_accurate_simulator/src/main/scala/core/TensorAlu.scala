...
class TensorAluPipelined(debug: Boolean = false)(implicit p: Parameters) extends TensorAluIfc {
  ...
  val valid_r1 = ShiftRegister(index_generator.io.valid, aluDataReadPipeDelay + 1, false.B, true.B)
...
// comment for split write
  ...
  // Do we need a bypass
  assert(!bypass_dst, cf"Bypass required on dst_idx read $dst_idx_r3 RAW with write $dst_idx_r4\n")
  assert(!bypass_src, cf"Bypass required on src_idx read $src_idx_r3 RAW with write $dst_idx_r4\n")
}
