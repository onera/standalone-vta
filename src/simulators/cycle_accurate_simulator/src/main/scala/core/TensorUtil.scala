...
/** TensorParams.
  ...
  * weights (wgt), biases (acc), and outputs (out). This is used to avoid doing
  * the same boring calculations over and over again.
  */
class TensorParams(tensorType: String = "none")(implicit p: Parameters)
  ...
  val writePipeLatency =
    tensorType match {
      case "inp"   => 0 // VME data load cmd write (per group)
      case "wgt"   => 0 // VME data load cmd write (per group)
      case "acc"   => 0 // VME data load cmd write (per group)
      case "fetch" => 0
      case "uop"   => 0
      case "out"   => 0
    }
  ...
  val readTensorLatency =
    tensorType match {
      case "inp"   => 0 // GEMM inp data read (per memsplit)
      case "wgt"   => 0
      case "acc"   => 0
      case "fetch" => 0
      case "uop"   => 0
  ...
  val readVMEDataLatency =
    tensorType match {
      case "inp"   => 0 // VME data signals delay
      case "wgt"   => 0
      case "acc"   => 0
      case "fetch" => 0
      case "uop"   => 0
  ...
  require(
    splitLength == 1 || splitWidth == 1,
    "-F- Can split only one dimension."
  )
...
/** TensorMaster.
  *
  * This interface issue read and write tensor-requests to scratchpads. For
  * example, The TensorGemm unit uses this interface for managing the inputs
  * (inp), weights (wgt), biases (acc), and outputs (out).
  */
class TensorMaster(tensorType: String = "none")(implicit p: Parameters)
    extends TensorParams(tensorType) {
  val rd = Vec(
    splitLength * splitWidth,
    new Bundle {
      val idx = ValidIO(UInt(memAddrBits.W))
      val data =
        Flipped(ValidIO(Vec(lenSplit, Vec(widthSplit, UInt(tensorElemBits.W)))))
    }
  )
  val wr = Vec(
    splitLength * splitWidth,
...
/** TensorClient.
  *
  * This interface receives read and write tensor-requests to scratchpads. For
  * example, The TensorLoad unit uses this interface for receiving read and
  * write requests from the TensorGemm unit.
  */
class TensorClient(tensorType: String = "none")(implicit p: Parameters)
  ...
  val rd = Vec(
    ...
    new Bundle {
      ...
      val data = ValidIO(Vec(lenSplit, Vec(widthSplit, UInt(tensorElemBits.W))))
    }
  )
  val wr = Vec(
    splitLength * splitWidth,
    Flipped(ValidIO(new Bundle {
      val idx = UInt(memAddrBits.W)
      val data = Vec(lenSplit, Vec(widthSplit, UInt(tensorElemBits.W)))
    }))
  )
...
/** TensorMasterData.
  ...
  * convention is based on the TensorMaster interface, which means this is an
  * input. This interface is used on datapath only module such MatrixVectorCore
  * or AluVector.
  */
...
/** TensorClientData.
  *
  * This interface is only used for datapath only purposes and the direction
  * convention is based on the TensorClient interface, which means this is an
  * output. This interface is used on datapath only module such MatrixVectorCore
  * or AluVector.
  */
