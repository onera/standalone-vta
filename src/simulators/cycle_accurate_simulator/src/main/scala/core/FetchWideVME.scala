...
/** Fetch.
  ...
  */
class FetchWideVME(debug: Boolean = false)(implicit p: Parameters)
    extends Module {
  ...
  val inst_q = Seq.fill(tensorsInClNb) {
    require(
      (tp.memDepth / tensorsInClNb) * tensorsInClNb == tp.memDepth,
  ...
  when(start) {
    ...
    assert(
      reset.asBool || state === sIdle || queueCount =/= 0.U ||
...
class GenVMECmdWideFetch(debug: Boolean = false)(implicit p: Parameters)
  ...
  when(io.start) {
    val tp = new TensorParams("fetch")
    assert(
      io.ins_baddr % (tp.tensorSizeBits / 8).U === 0.U,
      "-F- Fetch DRAM address expected to be tensor size aligned."
    )
  }
