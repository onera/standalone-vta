...
class MAC(
  ...
  val add = Wire(SInt(outBits.W)) // ADDED to have adder output

  mult := rA * rB
  add := rC +& mult // ADDED to have adder output
  val addV = if (flopIn) { add }
  else { RegNext(add) } // MODIFIED
...
/** Adder
  *
  * This unit wires input bits to an adder directly. The output comes out of
  * combinational logic without waiting for another cycle.
  */
...
class DotProduct(aBits: Int = 8, bBits: Int = 8, blockIn: Int = 16)
    extends Module {
  val errorMsg =
...
class MatrixVectorMultiplication(implicit p: Parameters) extends Module {
  ...
  val io = IO(new Bundle {
    val reset =
      Input(Bool()) // FIXME: reset should be replaced by a load-acc instr
  ...
  val acc = Seq.fill(batch)(
    Seq.fill(size)(Module(new Pipe(UInt(accBits.W), latency = 2)))
  )
  ...
  for (b <- 0 until batch) {
    for (i <- 0 until size) {
      acc(b)(
        i
      ).io.enq.valid := io.inp.data.valid & io.wgt.data.valid & io.acc_i.data.valid & ~io.reset
      acc(b)(i).io.enq.bits := io.acc_i.data.bits(b)(i)
      for (j <- 0 until size) {
        dot(b)(i).io.a(j) := io.inp.data.bits(b)(j).asSInt
        dot(b)(i).io.b(j) := io.wgt.data
          .bits(i)(j)
          .asSInt // all batches get the same weight - reuse
      }
...
class MatrixVectorMultiplicationBypass(implicit p: Parameters) extends Module {
  ...
  val last_acc_write = Seq.fill(batch)(Seq.fill(blockOut) {
    Reg(SInt(accBits.W))
  })
...
/** TensorGemmSimple
  ...
  * micro-ops (uops) which are used to read inputs, weights and biases, and
  * writes results back to the acc and out scratchpads.
  *
  * Also, TensorGemmSimple uses the reset field in the Gemm instruction to clear
  * or zero-out the acc-scratchpad locations based on the micro-ops.
  */
class TensorGemmSimple(debug: Boolean = false)(implicit p: Parameters)
    extends TensorGemmIfc {
  ...
  if (debug) {
    ...
    io.inp.rd(0).data.bits.zipWithIndex.foreach { case (r, i) =>
      when(io.inp.rd(0).data.valid && ~dec.reset) {
        printf("[TensorGemm] [inp] i:%x val:%x\n", i.U, r.asUInt)
      }
    }

    io.wgt.rd(0).data.bits.zipWithIndex.foreach { case (r, i) =>
...
class TensorGemmPipelinedSplit(implicit p: Parameters) extends TensorGemmIfc {
  ...
  val acc_idx_pipe = Module(
    new Pipe(
      chiselTypeOf(io.acc.rd(0).idx.bits),
      latency = 1 /* borrow 1 stage to split*/ + scratchpadReadLatency
    )
  )
  ...
  require(
    io.inp.splitWidth == 1 && io.inp.splitLength == 1,
    "-F- Input split read not supported"
  )
  ...
  val wrpipe0 = Module(
    new Pipe(
      chiselTypeOf(io.acc.wr(0).bits.idx),
      latency = 2 + scratchpadReadLatency
    )
  )
  ...
  val wrpipeNs = Module(
    new Pipe(chiselTypeOf(io.acc.wr(0).bits.idx), latency = 1)
  )
  ...
  val inpRdData0 = for (idx <- 0 until splitFactorL0) yield {
    if (scratchpadReadLatency > 0) RegNext(io.inp.rd(0).data)
    else io.inp.rd(0).data
  }
  ...
  val mvc = for (idx <- 0 until numMVMs) yield {
    Module(new MatrixVectorMultiplicationBypass)
  }
  ...
  for (idx1 <- 0 until numMVMs) {
    ...
    mvc(idx1).io.bypass_cond :=
      wrpipe(idx1).io.deq.bits === wrpipe2.io.deq.bits && wrpipe(
        idx1
      ).io.deq.valid && wrpipe2.io.deq.valid

    // borrow one stage from reset_pipe and split per group
    mvc(idx1).io.valid_reset := RegNext(
      reset_pipe.io.deq.bits & reset_pipe.io.deq.valid,
      init = false.B
    )
    // wire to each mvm
    mvc(idx1).io.inp.data :=
      ShiftRegister(
        inpRdData0(idx1 / splitFactorL1),
        mvmInpRdLatency
      ) // delay to deliver over distance
    mvc(idx1).io.wgt.data := io.wgt
      .rd(idx1)
      .data // wgt read idx is delayed instead of data
    mvc(idx1).io.acc_i.data.valid := io.acc.rd(idx1).data.valid
    assert(
      mvc(idx1).io.acc_o.data.valid === (wrpipe(idx1).io.deq.valid | mvc(
        idx1
      ).io.valid_reset)
    )
...
// comment to split write out
  if (numMVMs > 1) {
    for (idx1 <- 1 until numMVMs) {
      assert(
        mvc(idx1).io.out.data.valid === mvc(idx1 - 1).io.out.data.valid,
        "-F- Out split write is not supported"
      )
    }
  }
  ...
  for (idx3 <- 0 until numMVMs) {
    for (idx1 <- 0 until io.out.tensorLength) {
      for (idx2 <- 0 until io.out.tensorWidth / numMVMs) {
        outData(idx1)(idx3 * io.out.tensorWidth / numMVMs + idx2) := mvc(
          idx3
        ).io.out.data.bits(idx1)(idx2)
      }
    }
  }
  io.out.wr(0).bits.data := outData
  io.out.wr(0).valid := wrpipeNs.io.deq.valid && mvc(
    io.acc.closestIOGrpIdx
  ).io.out.data.valid
