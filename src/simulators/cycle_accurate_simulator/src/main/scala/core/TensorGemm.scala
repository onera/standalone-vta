...
class MAC(aBits: Int = 8, bBits: Int = 8, cBits: Int = 16, flopIn: Boolean = false) extends Module {
  ...
  val addV = if (flopIn) {add} else {RegNext(add)} // MODIFIED
...
class TensorGemmPipelinedSplit (implicit p: Parameters) extends TensorGemmIfc {
  ...
  val delayed_valid = ShiftRegister(m.io.valid, uopReadLatency + 1, false.B, true.B)
  ...
  val uop_valid = ShiftRegister(delayed_valid, inpReadIdxLatency, false.B, true.B)
