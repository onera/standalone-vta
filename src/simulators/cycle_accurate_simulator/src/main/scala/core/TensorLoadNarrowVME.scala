...
class TensorLoadNarrowVME(tensorType: String = "none", debug: Boolean = false)(
  ...
  fillPadding.io.start := RegNext(
    io.start,
    init = false.B
  ) // stage it to move from instr que
  ...
  val splitMemBlockFactor = if (splitDataFactor > memSizeRatio) {
    require(
      (splitDataFactor / memSizeRatio) * memSizeRatio == splitDataFactor,
  ...
  val groupMemBlockFactor = if (splitDataFactor > memSizeRatio) {
    ...
    require(
      (memSizeRatio / splitDataFactor) * splitDataFactor == memSizeRatio,
  ...
  require(
    splitDataFactor * groupMemBlockFactor == memSizeRatio * splitMemBlockFactor,
  ...
  val splitFactorL1 =
    pow(2, log2Ceil(memSizeRatio) - log2Ceil(memSizeRatio) / 2).toInt
  ...
  val waddrDirect = (VecInit((for (grIdx <- 0 until splitDataFactor) yield {
    ...
    .asTypeOf(
      Vec(
        ...
        chiselTypeOf(io.tensor.wr(0).bits.idx)
      )
    )

  val waddr = Wire(
    Vec(memSizeRatio * splitMemBlockFactor, chiselTypeOf(waddrTensInstrTmp))
  )
  ...
  for (j <- 0 until memSizeRatio * splitMemBlockFactor) {
    ...
    wdataTensInstr(j) := Mux(
      ...
      ShiftRegister(
        padValue /* a single group total data bits */,
        writePipeLatency
      ),
  ...
  val wdataDirect = VecInit(
    (for (grIdx <- 0 until splitDataFactor) yield {
      io.tensor.wr(grIdx).bits.data
    }).flatMap(elem =>
      for (k <- 0 until groupMemBlockFactor) yield {
        elem.asTypeOf(
          Vec(
            groupMemBlockFactor,
            UInt((tp.memBlockBits / splitMemBlockFactor).W)
  ...
  if (debug) {
    ...
    when(vmeDataFirePipe) {
      printf(
        cf"[TensorLoad] $tensorType data rdDataDestCol: ${rdDataDestCol} rdDataDestIdx: ${rdDataDestIdx} \n"
      )
    }
  }
...
class ZeroPadding(tensorType: String = "none", debug: Boolean = false)(implicit
  ...
  val zpColIdx = Reg(
    UInt((dec.xpad_0.getWidth + dec.xsize.getWidth + dec.xpad_1.getWidth).W)
  )
  val zpNewFillBlock = Wire(
    Bool()
  ) // separate new fill block <-> inside block  row change and column idx calculation
  // Define padding area iterators
  val zpRowIdx = Reg(
    UInt((dec.ypad_0.getWidth + dec.ysize.getWidth + dec.ypad_1.getWidth).W)
  ) // current padding row
  ...
  val zpTopLastIdx =
    dec.xpad_0 + dec.xsize + dec.xpad_1 - 1.U // last index of total width
  val zpWideLineEnd =
    (zpState === sZpSideBoth || zpState === sZpSideRight) && zpColIdx === zpTopLastIdx
...
class ReadVMEData(tensorType: String = "none", debug: Boolean = false)(implicit
  ...
  require(
    pow(2, log2Ceil(tp.memBlockBits)) == tp.memBlockBits,
    "-F- Tensor bit size must be 2^. Using shift and bits to divide."
  )
  require(
    tp.tsSizeRatio >= 1,
  ...
  val rdDataDestCol = Wire(
    UInt(blkOffsetWidth.W)
  ) // this is an index of a cl in a tensor
  val rdDataDestIdx = Wire(
    UInt(M_SRAM_OFFSET_BITS.W)
  ) // this is an index of a tensor
  io.vmeData.ready := true.B // always ready to read VME data
...
class GenVMECmd(tensorType: String = "none", debug: Boolean = false)(implicit
  ...
  val firstMaxTransfer =
    (maxTrBytes - rdCmdExtAddr % maxTrBytes) >> (log2Ceil(mp.dataBits) - 3)
  ...
  val startIssueCmdRead = Wire(
    Bool()
  ) // First transaction in dec.xsize transfer
  ...
  when(
    (blocksReadNb === blocksReadSize - readLen) && (srcRowIdx =/= dec.ysize - 1.U) && io.vmeCmd.fire
  ) {
    stride := true.B
  }.otherwise {
    stride := false.B
  ...
  assert(
    !io.isBusy || blocksReadSize >= blocksReadNb
  ) // define how many block to read at this cycle
  ...
  val totalWidth =
    dec.xsize + dec.xpad_0 + dec.xpad_1 // width of scratchpad matrix in tensors
  ...
  val rdCmdDestBlockIdx = Wire(
    UInt(blkIdxWdth.W)
  ) // dataBits size block index in a scratchpad
  val rdCmdDestBlockIdxNext = Reg(
    chiselTypeOf(rdCmdDestBlockIdx)
  ) // dataBits size block index in a scratchpad
  ...
  // read-from-dram
  require(
    io.vmeCmd.bits.tag.getWidth >= rdCmdDestBlockIdx.getWidth,
  ...
  assert(
    !io.vmeCmd.valid || ((readLen << log2Ceil(
      mp.dataBits / 8
    )) <= (maxTrBytes - rdCmdExtAddr % maxTrBytes)),
