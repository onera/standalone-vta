...
class TensorLoadWideVME(tensorType: String = "none", debug: Boolean = false)(
  ...
  val vmeDataBitsPipe =
    ShiftRegister(io.vme_rd.data.bits, readVMEDataLatency, true.B)
  val vmeDataValidPipe =
    ShiftRegister(io.vme_rd.data.valid, readVMEDataLatency, false.B, true.B)
  val vmeDataReadyPipe =
    ShiftRegister(io.vme_rd.data.ready, readVMEDataLatency, true.B, true.B)
  ...
  require(
    mp.dataBits >= tp.tensorSizeBits,
    "-F- Chacheline width must be larger than tensor bit size"
  )
  require(
    pow(2, log2Ceil(mp.dataBits)) == mp.dataBits,
    "-F- Chacheline width must be pow of 2"
  )
  require(
    pow(2, log2Ceil(tp.tensorSizeBits)) == tp.tensorSizeBits,
  ...
  assert(
    !readData.io.vmeData.valid || readData.io.vmeData.ready,
  ...
  val isZeroPadWrite =
    fillPadding.io.tensorIdx.valid // Store zero filled tensor, zpDestIdx is valid
  val zpDestIdx = fillPadding.io.tensorIdx.bits >> tensorsInClNbWidth // SP idx
  val zpDestMask =
    if (tensorsInClNb == 1) 1.U
    else
      UIntToOH(
        fillPadding.io.tensorIdx.bits(tensorsInClNbWidth - 1, 0)
  ...
  val directWrMask = for (grpIdx <- 0 until splitDataFactor) yield {
    Mux(
      ...
      if (tensorsInClNb == 1) 1.U
        ...
        UIntToOH(
          io.tensor.wr(grpIdx).bits.idx(tensorsInClNbWidth - 1, 0)
        ), // tensor in a memory line
      0.U
    )
  }
  ...
  val wmask = Wire(
    Vec(tensorsInClNb * splitDataFactor * splitMemFactor, Bool())
  )
  for (i <- 0 until tensorsInClNb) {
    for (grpIdx <- 0 until splitDataFactor) {
      for (memIdx <- 0 until splitMemFactor) { // duplicate control
        wmask(
          i * splitDataFactor * splitMemFactor + grpIdx * splitMemFactor + memIdx
        ) :=
          Mux(
            ShiftRegister(state === sIdle, writePipeLatency, true.B, true.B),
            directWrMask(grpIdx)(i),
            Mux(
              ShiftRegister(isZeroPadWrite, writePipeLatency, false.B, true.B),
    ...
    for (grpIdx <- 0 until splitDataFactor) {
      ...
      wdata(i * splitDataFactor + grpIdx) := Mux(
        ...
        Mux(
          ...
          ShiftRegister(
            (rdDataWrData(i)
              .asTypeOf(Vec(splitDataFactor, UInt(groupSizeBits.W))))(grpIdx),
            writePipeLatency
          )
        )
      )
  ...
  val widx = Wire(
    Vec(
      tensorsInClNb * splitDataFactor * splitMemFactor,
      ...
      for (memIdx <- 0 until splitMemFactor) { // duplicate control
        when(
          ...
          tensorFile(
            ...
            wdata(i * splitDataFactor + grpIdx)
              .asTypeOf(Vec(splitMemFactor, UInt(memSizeBits.W)))(memIdx)
          )
        }
      }
    }
  ...
  for (grpIdx <- 0 until splitDataFactor) {
    ...
    val rMask =
      Mux(
        ...
        if (tensorsInClNb == 1) 1.U
          ...
          UIntToOH(
            io.tensor.rd(grpIdx).idx.bits(tensorsInClNbWidth - 1, 0)
          ), // tensor in a memory line
        0.U
      )
    ...
    val rdataVec = for (i <- 0 until tensorsInClNb) yield {
      VecInit(for (memIdx <- 0 until splitMemFactor) yield {
        tensorFile(
          ...
          ShiftRegister(
            VecInit(rMask.asBools)(i),
            tp.readTensorLatency,
            false.B,
            true.B
          )
        )
    ...
    io.tensor.rd(grpIdx).data.bits := rdata.asTypeOf(
      io.tensor.rd(grpIdx).data.bits
    )

    val rvalid = ShiftRegister(
      io.tensor.rd(grpIdx).idx.valid,
      tp.readTensorLatency + 1,
      false.B,
      true.B
    )
  ...
  io.done := ShiftRegister(localDone, writePipeLatency, false.B, true.B)
}
...
class ReadVMEDataWide(tensorType: String = "none", debug: Boolean = false)(
  ...
  require(
    pow(2, log2Ceil(tp.tensorLength)) == tp.tensorLength,
    "-F- Tensor length must be 2^. Using shift and bits to divide."
  )
  val blkIdxWdth = log2Ceil(
    tp.memDepth
  ) // the size of scratchpad in cache lines
  ...
  val rdDataMaskDecodeWidth =
    if (wmaskWidth == 1) 1 else (log2Ceil(wmaskWidth) + 1)
  val rdDataElemIdx =
    vmeTagDecode(vmeTagDecode.getWidth - 1, 2 * rdDataMaskDecodeWidth)
  val rdFstOffsetNb = if (rdDataMaskDecodeWidth == 0) {
    0.U
  } else {
    val readOffset =
      vmeTagDecode(2 * rdDataMaskDecodeWidth - 1, rdDataMaskDecodeWidth)
    readOffset
  }
  val rdLstNb = if (rdDataMaskDecodeWidth == 0) {
    ...
    assert(
      !io.vmeData.valid || readNb > 0.U,
      "-F- Expecting some elements to read"
    )
    readNb
  }
  ...
  val rdDataElemDestIdx = Wire(
    UInt(tp.memAddrBits.W)
  ) // this is an idx  of a tensor
  ...
  val vmeTagDecodeLastValidNext =
    RegNext(next = vmeTagDecodeLastValid, init = false.B)
  when(io.start) {
  ...
  val wmaskSel =
    Mux(
      ...
      Mux(
        isFirstPulse,
        wrMask1st,
        Mux(isLastPulse, wrMaskLast, ((1 << wmaskWidth) - 1).U)
      )
    )
  ...
  val srcData = io.vmeData.bits.data
    .asTypeOf(Vec(tp.clSizeRatio, UInt(tp.tensorSizeBits.W)))
  val srcOffset = Wire(
...
class GenVMECmdWide(tensorType: String = "none", debug: Boolean = false)(
  ...
  val dramClAddrAlignNotMask =
    ((BigInt(1) << log2Ceil(clBytes)) - 1).U.asTypeOf(xferElemInitAddr)
  ...
  val rdLineElemBeginAddr = Reg(
    UInt(mp.addrBits.W)
  ) // DRAM address of xsize tensors memory line
  val rdLineClBeginAddr = rdLineElemBeginAddr & dramClAddrAlignMask
  // begin of the next DRAM memory line
  val nextLineBeginElemAddr =
    rdLineElemBeginAddr + (io.xstride << log2Ceil(elemBytes))
  ...
  assert(
    rd1stPulseOffsetBytes >> log2Ceil(elemBytes) <= tp.clSizeRatio.U,
  ...
  val rdLineClNb =
    Mux(
      (rdLineBytes + rd1stPulseOffsetBytes) % clBytes.U === 0.U,
      rdLineClNbTmp,
      rdLineClNbTmp + 1.U
    )
  ...
  assert(
    rdLastPulseBytes >> log2Ceil(elemBytes) <= (clBytes / elemBytes).U,
  ...
  rdLastPulseTensNb := Mux(
    rdLastPulseTensNbTmp === 0.U,
    (clBytes / elemBytes).U,
    rdLastPulseTensNbTmp
  )
  ...
  val rdCmdStartIdx = Reg(
    UInt(log2Ceil(tp.memDepth).W)
  ) // Scratchpad data block index for the first transaction
  val commandsDone = RegInit(true.B) // Done generating VME commands
  // counts the number of CLs read in a xsize line
  val clReadIdx = Reg(
    UInt((io.xsize.getWidth + log2Ceil(elemBytes) - log2Ceil(clBytes)).W)
  )
  ...
  when(
    (clReadIdx === rdLineClNb - rdLen) && (dramLineIdx =/= io.ysize - 1.U) && io.updateState
  ) {
    stride := true.B
  }.otherwise {
    stride := false.B
  ...
  val rdCmd1stPluseOffsetTensNb = Wire(chiselTypeOf(rd1stPulseOffsetTensNb))
  val rdCmdLastPluseTensNb = Wire(chiselTypeOf(rdLastPulseTensNb))
  when(newReadRow) {
    // first read in line
  ...
  assert(
    !io.isBusy || rdLineClNb >= clReadIdx
  ) // define how many cachelines to read at this cycle
  ...
  val totalWidth =
    io.xsize + io.xpad_0 + io.xpad_1 // width of scratchpad matrix in tensors
  ...
  val rdCmdDestElemIdx = Wire(
    UInt(tp.memAddrBits.W)
  ) // element(tensor) size block index in a scratchpad
  val rdCmdDestElemIdxNext = Reg(chiselTypeOf(rdCmdDestElemIdx))
  ...
  val rdCmdTransactionTensNb =
    (rdLen << log2Ceil(clBytes / elemBytes)) - rdCmd1stPluseOffsetTensNb
  ...
  // read-from-dram
  require(
    io.vmeCmd.bits.tag.getWidth >= rdCmdDestElemIdx.getWidth +
  ...
  assert(
    !io.vmeCmd.valid || ((rdLen << log2Ceil(
      clBytes
    )) <= maxTrBytes - rdLineAddr % maxTrBytes),
    s"-F- ${tensorType} DRAM page alignment failure. DRAM " +
      s"address + len overlaps mp.lenBits*memBlockSize alignment %x %x",
    rdLineAddr,
    rdLen
  )
  io.vmeCmd.bits.tag := Cat(
    rdCmdDestElemIdx,
    Cat(rdCmd1stPluseOffsetTensNb, rdCmdLastPluseTensNb)
  )
  io.readLen := rdLen
