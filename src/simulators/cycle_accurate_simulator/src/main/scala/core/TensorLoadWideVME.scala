...
class TensorLoadWideVME(tensorType: String = "none", debug: Boolean = false)(
  ...
  val vmeDataBitsPipe = ShiftRegister(io.vme_rd.data.bits, readVMEDataLatency, true.B)
  val vmeDataValidPipe = ShiftRegister(io.vme_rd.data.valid, readVMEDataLatency, false.B, true.B)
  val vmeDataReadyPipe = ShiftRegister(io.vme_rd.data.ready, readVMEDataLatency, true.B, true.B)
        ...
        wmask(i*splitDataFactor*splitMemFactor + grpIdx * splitMemFactor + memIdx) :=
          Mux(
            ...
            Mux(
              ...
              Mux(
                ShiftRegister(vmeDataFirePipe, writePipeLatency, false.B, true.B),
  ...
  if (debug) {
    when(isZeroPadWrite) {
      printf(cf"[TensorLoad] $tensorType isZeroPadWrite data zpDestIdx: ${zpDestIdx} \n")
    }
  }
  ...
  for (grpIdx <- 0 until splitDataFactor) {
      ...
      VecInit(for (memIdx <- 0 until splitMemFactor) yield {
        tensorFile(
          i*splitDataFactor*splitMemFactor + grpIdx * splitMemFactor + memIdx).read(
            ...
            ShiftRegister(VecInit(rMask.asBools)(i), tp.readTensorLatency, false.B, true.B))
      }).asUInt
    }
    ...
    val rvalid = ShiftRegister(
      io.tensor.rd(grpIdx).idx.valid, tp.readTensorLatency + 1, false.B, true.B)
