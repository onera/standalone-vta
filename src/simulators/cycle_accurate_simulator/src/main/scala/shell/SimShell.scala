...
/** VTAHost.
  ...
  * module and used to test host-to-VTA communication. This module should be
  * updated for testing hosts using a different bus protocol, other than AXI.
  */
...
/** VTAMem.
  *
  * This module translate the DPI protocol into AXI. This is a simulation only
  * module and used to test VTA-to-memory communication. This module should be
  * updated for testing memories using a different bus protocol, other than AXI.
  */
...
/** VTASim.
  *
  * This module is used to handle hardware simulation thread, such as halting or
  * terminating the simulation thread. The sim_wait port is used to halt the
  * simulation thread when it is asserted and resume it when it is de-asserted.
  */
class VTASim(implicit p: Parameters) extends Module {
  val sim_wait = FlatIO(Output(Bool()))
