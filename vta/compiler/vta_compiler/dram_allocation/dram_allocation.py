...
def dram_allocation(
    ...
    for obj_type, *rest in object_list:
        ...
        if obj_type == "INP":
            ...
            logical_divisor = np.dtype(out_dtype).itemsize * block_size
        else:
        ...
        obj_addr, current_dram_addr = addresses_computation(
            obj_type,
            obj_value,
            page_size,
            current_dram_addr,
...
def addresses_computation(
    ...
    obj_addr = {
        ...
        "logical_base_address": hex(
            (current_dram_addr - dram_offset) // logical_divisor
        ),
