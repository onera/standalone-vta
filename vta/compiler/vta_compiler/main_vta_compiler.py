...
def main(
    ...
    out_dtype = conf.data_type(vta_config_dict["LOG_OUT_WIDTH"])

    # Block size (hardware constraint)
    block_size = 2 ** vta_config_dict["LOG_BLOCK"]

    # SRAM memory capacity
    inp_buffer_size = conf.buffer_size(
        vta_config_dict["LOG_INP_BUFF_SIZE"],
        vta_config_dict["LOG_INP_WIDTH"],
        block_size,
    )
    wgt_buffer_size = conf.buffer_size(
        vta_config_dict["LOG_WGT_BUFF_SIZE"],
        vta_config_dict["LOG_WGT_WIDTH"],
        block_size * block_size,
    )
    acc_buffer_size = conf.buffer_size(
        vta_config_dict["LOG_ACC_BUFF_SIZE"],
        vta_config_dict["LOG_ACC_WIDTH"],
        block_size,
    )
    ...
    if flag_dict["doAlu"]:
        ...
        alu_list = ALU.alu_operations(
            alu_operations=alu_list, shape=(C_row, C_col), block_size=block_size
        )
    ...
    # Allocate the object
    base_addresses_list, updated_base_address = DA.dram_allocation(
        object_list,
    ...
    for mat_vec in flat_store_list:
        flat, pair = MTB.vectorMatrixToBlock(
            matrix_vector=mat_vec, block_size=block_size, nb_blocks_col=C_blocks_col
        )
        idx_to_store_to_sort = idx_to_store_to_sort + pair
    # Order the list
    if len(idx_to_store_to_sort) > 0:
        idx_to_store = SIS.sort_idx_to_store(
            idx_to_store=idx_to_store_to_sort,
            nb_col=C_blocks_col,
            block_size=block_size,
        )

    # Apply matrix partitioning (check is overfit then applies selected trategy)
    strategy, flag_dict = MP.matrix_partitioning(
        nb_A=nb_A,
    ...
    insn_buffer, uop_buffer = OP.operations_definition(
        strategy=strategy,
        dram_addresses=base_addresses_list,
    ...
    updated_addr, updated_base_address = DA.dram_allocation(
        object_list,
        base_addr=updated_base_address - 4,
        block_size=block_size,
        inp_dtype=inp_dtype,
        out_dtype=out_dtype,
        wgt_dtype=wgt_dtype,
    ...
    Y_matrix_file_path = filepath_definition(
        output_dir, "add_accumulator" + name + ".bin"
    )
    ...
    # TODO: FOR CHISEL
    # Dram allocation
    base_addresses_file_path = filepath_definition(
        output_dir, "memory_addresses" + name + ".csv"
    )
    with open(base_addresses_file_path, "w", newline="") as csvfile:
        ...
        for obj_addr in base_addresses_list:
            writer.writerow(
                [
                    obj_addr["type"],
                    obj_addr["physical_base_address"],
                    obj_addr["logical_base_address"],
                ]
            )
    ...
    if debug == True or summary == True:
        ...
        print(
            f"The compilation of '{name}' generates: \n\t {nb_steps} steps \n\t {nb_uop} UOPs \n\t {nb_insn} instructions"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("vta_ir", nargs="+")
    parser.add_argument("--debug", action="store_true")
    parser.add_argument("--summary", action="store_true")
    parser.add_argument("--dram-json", action="store_true")
    parser.add_argument("-o", "--output-dir")
    args = parser.parse_args()

    if args.output_dir:
        os.environ["VTA_COMPILER_OUT"] = args.output_dir
    ...
    vta_config_dict = parse_json_to_dict(args.config)
    ...
    vta_ir_files = sorted(args.vta_ir, key=os.path.basename)

    for i, vta_ir in enumerate(vta_ir_files):
        if debug or summary:
            print(f"-" * 50)
        ...
        base_address, name, steps, uop, insn, dram_state_dictionary = main(
            vta_config_dict,
            operations_dict,
            base_address,
            dram_offset,
    ...
    if dram_state_dictionary != False:
        ...
        with open(file_dram_state_path, "w") as f:
            json.dump(
                dram_state_dictionary, f, indent=2
            )  # indent=2 for better readibility
