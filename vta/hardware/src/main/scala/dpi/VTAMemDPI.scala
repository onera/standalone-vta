...
class VTAMemDPIToAXI(implicit val p: Parameters) extends Module {
  ...
  switch(rstate) {
    is(rIdle) {
      when(
        dpiReqQueue.io.deq.valid && dpiReqQueue.io.deq.bits.len =/= 0.U && counter === dpiDelay
      ) {
        rstate := readData
      }
    }
