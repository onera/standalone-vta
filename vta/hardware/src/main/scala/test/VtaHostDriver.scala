...
/** Synthesizable hardware per-layer VTA driver. Walks a baked-in layer ROM: for
  * each layer it programs the VCR pointer/value registers, writes ctrl=1 to
  * launch, polls ctrl bit1 (finish) over AXI-Lite, then advances. Raises `done`
  * after the last layer; raises `error` (latching `layerIdx`) if a layer never
