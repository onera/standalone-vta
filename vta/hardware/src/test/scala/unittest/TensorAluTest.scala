...
class TensorAluPipelinedTest extends AnyFlatSpecSim {
  ...
  val debug = false
  it should "run on some operation" in {
    simulate(new TensorAluPipelined) { c =>
      c.io.start.poke(0)
      ...
      val acc = IndexedSeq.tabulate(c.io.acc.rd(0).data.bits(0).size) { i =>
        BigInt(i)
      }
