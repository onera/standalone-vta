...
the Python VTA compiler (`vta/compiler`, a `uv` project) into a gitignored
...
no real bias) with `np.random`; `vta/fpga/test-tools/seed_runner.py` seeds numpy
...
Env to (re)generate: `uv sync` in `vta/compiler`, then `execstack -c` the
