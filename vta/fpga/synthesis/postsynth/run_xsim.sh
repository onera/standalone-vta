...
#
# Encodes the required file partition: the netlist leg compiles ONLY the TB-wrapper SV +
# the funcsim netlist + glbl, deliberately excluding the behavioral VTAShell.sv / Core RTL
# (which the netlist replaces) so VTAShell is not doubly defined. The behavioral leg
# compiles all emitted SV.
#
...
WAVE_NS=250000 # bounded wave window (ns); covers reset through first finish/timeout
...
[[ -n "$MODE" ]] || {
  echo "ERROR: pass --behavioral or --netlist <funcsim.v>" >&2
  usage 1
}
[[ -n "$OUT" ]] || {
  echo "ERROR: pass --out <dir>" >&2
  usage 1
}
[[ -n "${XILINX_VIVADO:-}" ]] || {
  echo "ERROR: XILINX_VIVADO unset - source Vivado settings64.sh first" >&2
  exit 1
}
command -v xvlog >/dev/null || {
  echo "ERROR: xvlog not on PATH - source Vivado settings64.sh" >&2
  exit 1
}
[[ -d "$TB" ]] || {
  echo "ERROR: TB emit dir not found: $TB (run ./mill ... emitVtaPostSynthTb)" >&2
  exit 1
}
[[ -f "$TB/VTAPostSynthTb.sv" ]] || {
  echo "ERROR: $TB/VTAPostSynthTb.sv missing" >&2
  exit 1
}
...
if [[ "$MODE" == netlist ]]; then
  [[ -f "$NETLIST" ]] || {
    echo "ERROR: netlist not found: $NETLIST (run the OOC synth first)" >&2
    exit 1
  }
...
#                    run in Vivado/GTKWave without re-simulating).
...
# read data the driver polls, status/vcr probes, m_axi_gmem). Default runs to completion
