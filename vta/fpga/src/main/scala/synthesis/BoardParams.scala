...
object BoardParams {
  ...
  def render(
    ...
    // NoC config deltas (Versal): when the board omits noc_config, fall back to
    // a default LPDDR4 dict, in a fixed order.
    val nocPairs =
    ...
    // recipe only reads them under is_versal).
    val versalLines = Seq(
    ...
    val exclItems = b.addressExcludes
      ...
      .mkString(" ")
    s"""# board_params.tcl - generated by the fpga.synthesis.BuildFpga task; do not edit by hand.
       |# board: ${b.name}
       |
