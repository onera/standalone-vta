...
class BinaryReaderTest extends AnyFlatSpec with should.Matchers {
  ...
  "BinaryReader" should "print the decoded instructions in console" in {
    val result = computeAddresses("examples_compute/instructions_lenet5_layer1.bin", DataType.INSN, 0)
    printMapLE(result)
  }
  ...
  it should "decode correctly the first vector of INP (16 Bytes) in a binary file" in {
    val result =  computeAddresses("examples_compute/input_lenet5_layer1.bin", DataType.INP, 0)
  ...
  it should "decode correctly the first vector of WGT (256 Bytes) in a binary file" in {
    val result =  computeAddresses("examples_compute/weight_lenet5_layer1.bin", DataType.WGT, 0)
  ...
  it should "return an error if the file formatting is wrong" in {
    val result = computeAddresses("examples_compute/compute_smm.json", DataType.INSN, 0)
